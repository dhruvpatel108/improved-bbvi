...
print(f"batched_y_meas.shape = {batched_y_meas.shape}, elbo_batched_y_meas.shape = {elbo_batched_y_meas.shape}")
...
gz_opt_np = np.zeros((config.n_samples, config.box_z_dim, config.box_x_dim))
...
save_dir = f"exps/Zdim{config.z_dim}_likevar{config.like_var}/bbvi/xi_{config.xi_sampling}/{config.optimizer}/BS{config.batch_size}_N{config.n_iter}_seed{config.seed_no}"
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
post_stats_plots(config.x_true, x_map, x_mean, x_std, save_dir=save_dir, file_postfix=f"_nf{config.batch_size*config.n_iter}", if_map=True)
