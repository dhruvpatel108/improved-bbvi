...
def pred_y_from_z_oldd(z, generator, F, min_value, max_value, batch_size, x_dim, like_dim):
...
def pred_y_from_z(z, generator, cnn_surrogate, min_value, max_value, batch_size, box_z_dim, box_x_dim):
    # generate batch_size samples
    z = torch.from_numpy(z).float().to(device)
    z = z.view(batch_size, -1)
    gz = ((generator(z)+1)/2)*(max_value-min_value) + min_value
    # rescale the generated samples to the range of [min_value, max_value]
    gz = gz.view(batch_size, *(1, box_z_dim, box_x_dim))
    # predict y
    y_pred = cnn_surrogate(gz).detach().cpu().numpy()
    return y_pred
