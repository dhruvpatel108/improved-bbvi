...
class Discriminator(nn.Module):
    def __init__(self, img_size, dim):
        ...
        self.image_to_features = nn.Sequential(
            nn.Conv2d(self.img_size[2], dim, (5, 9), (1, 1), (0, 0)),
            nn.LeakyReLU(0.2),
            nn.Conv2d(dim, 2 * dim, (5, 9), (1, 1), (0, 0)),
            nn.LeakyReLU(0.2),
            nn.Conv2d(2 * dim, 4 * dim, (4, 4), (2, 2), (1, 1)),
            nn.LeakyReLU(0.2),
            nn.Conv2d(4 * dim, 8 * dim, (4, 4), (2, 2), (1, 1)),
            nn.LeakyReLU(0.2),
            nn.Conv2d(8 * dim, 16 * dim, (4, 4), (2, 2), (1, 1)),
            nn.LeakyReLU(0.2),
            nn.Conv2d(16 * dim, 16 * dim, (4, 4), (2, 2), (1, 1)),
            nn.LeakyReLU(0.2)
        )

        # n convolutions of stride 2, i.e. halving of size everytime
        # So output size will be 8 * (img_size / 2 ^ n) * (img_size / 2 ^ n)
        output_size = 16 * dim * 2 * 4 #int(img_size[1] / 16) * int(img_size[1] / 16)
        self.features_to_output = nn.Sequential(
            nn.Linear(output_size, 2 * dim),
            nn.LeakyReLU(0.2),
            nn.Linear(2 * dim, 1)
        )
