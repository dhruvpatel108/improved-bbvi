...
# ==============================================================================
def mcmc_post_processing(z_post_samples, save_dir_prefix):
    save_dir = f"exps/id{config.test_img_index}_k{config.k}_noisevar{config.noise_var}_likevar{config.like_var}/Zdim{config.z_dim}/{save_dir_prefix}/N{config.n_mcmc_samples}_PropVar{config.mcmc_prop_var}"
    ...
    gz = np.zeros((zz_post_samples.shape[0], config.x_dim, config.x_dim))
    for ii in range(n_batches):
        zz_post_torch = torch.autograd.Variable(torch.tensor(zz_post_samples[ii*mcmc_batch_size:(ii+1)*mcmc_batch_size, :], dtype=torch.float32))
        gz[ii*mcmc_batch_size:(ii+1)*mcmc_batch_size, :, :] = ((((torch.squeeze(config.generator(zz_post_torch)) + 1.) /2)*(config.max_value-config.min_value)) + config.min_value).detach().numpy()
