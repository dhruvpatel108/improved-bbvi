...
def log_var_fn_gamma(xi, batched_gamma_t):
    ...
    log_var = -np.sum(batched_gamma_t, axis=1) - (0.5*np.linalg.norm(xi, axis=1)**2)
    return log_var



def bbvi_gradient(xi, xi_orig_1d, x, mu_t, gamma_t, batched_mu_t, batched_gamma_t, prior, likelihood, if_fd_correction=False, if_cv=False):
    ...
    log_prior_vec = np.expand_dims(prior.log_prior_batch(x), axis=1)
    log_like_vec = np.expand_dims(likelihood.log_likelihood_batch(x), axis=1)
    log_var_vec = np.expand_dims(log_var_fn_gamma(xi, batched_gamma_t), axis=1)
    grad_mu_vanilla = (grad_logq_mu.T @ (log_prior_vec + log_like_vec - log_var_vec))/config.batch_size
    grad_gamma_vanilla = (grad_logq_gamma.T @ (log_prior_vec + log_like_vec - log_var_vec))/config.batch_size
    ...
    if if_fd_correction==True and if_cv==False:
        grad_mu_fd, grad_gamma_fd = fd_of_log_prior_like_var(xi_orig_1d, config.epsilon, mu_t, gamma_t, prior, likelihood)    # [config.z_dim, 1] each
        ...
        grad_mu1 = (grad_logq_mu.T @ (log_prior_vec + log_like_vec - log_var_vec - first_order_correction_mu))/config.batch_size
        grad_gamma1 = (grad_logq_gamma.T @ (log_prior_vec + log_like_vec - log_var_vec - first_order_correction_gamma))/config.batch_size

        grad_mu = grad_mu1 + grad_mu_fd
        #print(f"grad_mu_fd: {grad_mu_fd}")
        #print(f"(x-batched_mu_t): {(x-batched_mu_t)}, grad_mu_fd: {grad_mu_fd}, first_order_correction_mu: {first_order_correction_mu}")
        #print(f"log_prior_vec: {log_prior_vec}, log_like_vec: {log_like_vec}, log_var_vec: {log_var_vec}, \
        #        first3terms: {log_prior_vec + log_like_vec - log_var_vec}, first_order_correction_mu: {first_order_correction_mu}")
        #print(f"grad_mu1: {grad_mu1}, grad_mu_vanilla: {grad_mu_vanilla}, grad_logq_mu: {grad_logq_mu}")
        #print(f"gamma info ===")
        #print(f"gamma_t: {gamma_t}")
        #print(f"(x-batched_gamma_t): {(x-batched_gamma_t)}, grad_gamma_fd: {grad_gamma_fd}, first_order_correction_gamma: {first_order_correction_gamma}")
        #print(f"grad_gamma_fd: {grad_gamma_fd}")        
        #print(f"grad_gamma1: {grad_gamma1}, grad_gamma_vanilla: {grad_gamma_vanilla}")
    ...
    if if_fd_correction==False and if_cv==True:
        ...
        f_mu = grad_logq_mu * (log_prior_vec + log_like_vec - log_var_vec)     # [batch_size, config.z_dim]
        #h_mu = grad_logq_mu
        expec_x2_mu = np.mean(f_mu*grad_logq_mu, axis=0)
        expec_squared_mu = np.mean(f_mu, axis=0)*np.mean(grad_logq_mu, axis=0)
        cov_fh_mu = (expec_x2_mu - expec_squared_mu)*(config.batch_size/(config.batch_size-1))
        ...
        f_gamma = grad_logq_gamma * (log_prior_vec + log_like_vec - log_var_vec)     # [batch_size, config.z_dim]
        ...
        cov_fh_gamma = (expec_x2_gamma - expec_squared_gamma)*(config.batch_size/(config.batch_size-1))
    ...
    if if_fd_correction==True and if_cv==True:
        ...
        if config.z_dim==1:
            ...
            denominator_mu = np.diag(np.cov(grad_logq_mu.T))  
        f_mu = grad_logq_mu * (log_prior_vec + log_like_vec - log_var_vec - first_order_correction_mu)     # [batch_size, config.z_dim]
        #h_mu = grad_logq_mu
        expec_x2_mu = np.mean(f_mu*grad_logq_mu, axis=0)
        expec_squared_mu = np.mean(f_mu, axis=0)*np.mean(grad_logq_mu, axis=0)
        cov_fh_mu = (expec_x2_mu - expec_squared_mu)*(config.batch_size/(config.batch_size-1))
        ...
        if config.z_dim==1:
            ...
            denominator_gamma = np.diag(np.cov(grad_logq_gamma.T))
        f_gamma = grad_logq_gamma * (log_prior_vec + log_like_vec - log_var_vec - first_order_correction_gamma)     # [batch_size, config.z_dim]
        #h_sigma = grad_logq_sigma
        expec_x2_gamma = np.mean(f_gamma*grad_logq_gamma, axis=0)
        expec_squared_gamma = np.mean(f_gamma, axis=0)*np.mean(grad_logq_gamma, axis=0)
        cov_fh_gamma = (expec_x2_gamma - expec_squared_gamma)*(config.batch_size/(config.batch_size-1))
...
def fd_of_log_prior_like_var(xi_orig_1d, epsilon, mu, gamma, prior, likelihood):
    # mu: [config.z_dim, 1]
    # gamma: [config.z_dim, 1]    
    gradL_mu_fd = np.zeros([config.z_dim, 1])
    for ii in range(config.z_dim):
        ...
        # variational
        log_var_plus = -(0.5*config.z_dim*np.log(2*np.pi)) - np.sum(gamma) - (0.5*np.linalg.norm((xn_plus - mu)/np.exp(gamma))**2)
        log_var_minus = -(0.5*config.z_dim*np.log(2*np.pi)) - np.sum(gamma) - (0.5*np.linalg.norm((xn_minus - mu)/np.exp(gamma))**2)
        # gradient       
        gradL_mu_fd[ii] = ((log_prior_plus + log_like_plus - log_var_plus) - (log_prior_minus + log_like_minus - log_var_minus))/(2*epsilon)


    gradL_gamma_fd = np.zeros([config.z_dim, 1])
    for ii in range(config.z_dim):
        ...
        log_like_minus = likelihood.log_likelihood_sample(xn_minus)
        # variational
        log_var_plus = -(0.5*config.z_dim*np.log(2*np.pi)) - np.sum(gamma_plus) - (0.5*np.linalg.norm((xn_plus - mu)/np.exp(gamma_plus))**2)
        log_var_minus = -(0.5*config.z_dim*np.log(2*np.pi)) - np.sum(gamma_minus) - (0.5*np.linalg.norm((xn_minus - mu)/np.exp(gamma_minus))**2)
        # gradient
...
def fd_gradient(epsilon, mu, gamma, prior, likelihood):
    """
    finite difference gradient check
    """
    batch_size = 10000
    #xi_orig = np.random.multivariate_normal(mean=np.zeros(z_dim), cov=np.eye(z_dim), size=int(batch_size/2))
    #xi_neg = -xi_orig
    #xi = np.concatenate((xi_orig, xi_neg), axis=0)
    xi = np.random.multivariate_normal(mean=np.zeros(config.z_dim), cov=np.eye(config.z_dim), size=batch_size)
    gradL_mu_fd = np.zeros([config.z_dim, 1])
    gradL_gamma_fd = np.zeros([config.z_dim, 1])
    for ii in range(config.z_dim):
        mu_plus = np.copy(mu)
        ...
        mu_plus_2d = np.repeat(mu_plus.T, batch_size, axis=0)
        mu_minus_2d = np.repeat(mu_minus.T, batch_size, axis=0)
        gamma_2d = np.repeat(gamma.T, batch_size, axis=0)
        ...
        elbo_plus = L_batch_plus/batch_size
        elbo_minus = L_batch_minus/batch_size
    ...
    for ii in range(config.z_dim):
        ...
        gamma_plus_2d = np.repeat(gamma_plus.T, batch_size, axis=0)
        gamma_minus_2d = np.repeat(gamma_minus.T, batch_size, axis=0)
