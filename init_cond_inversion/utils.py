...
def pred_y_from_z(z, generator, F, min_value, max_value, batch_size, x_dim, like_dim):
    # generate batch_size samples
    z = torch.from_numpy(z).float().to(device)
    z = z.view(batch_size, -1)
    gz = generator(z)
    # rescale the generated samples to the range of [min_value, max_value]
    gz = ((gz+1)/2)*(max_value-min_value) + min_value
    gz = gz.view(batch_size, *(x_dim, x_dim, 1))
    gz = gz.detach().cpu().numpy()
    # extract the middle part of the generated samples
    gz_vec = gz[:, 1:-1, 1:-1, 0].reshape([batch_size, like_dim])
    # predict y
    y_pred = np.matmul(F, gz_vec.T).T
    return y_pred
...
def data_processing(xi_sampling, z_dim, batch_size, prior_mean_vec, prior_var_vec, like_var_vec, y_meas):
    ...
    if batch_size==1:
        ...
        elif xi_sampling=="normal_nonsymmetric":
        ...
        #xx = xi*sigma + mu_2d
            ...
            start_pt = stats.norm.ppf(0.1, loc=0, scale=1)
...
def post_stats_plots(x_true, y_true, y_meas, x_map, x_mean, x_std, save_dir, file_postfix, if_map):
    cmap = "inferno"
    ...
    fig.colorbar(im, cax=cax, orientation="vertical")
    if if_map:
        im = ax[1, 0].imshow(x_map, cmap=cmap, vmin=x_true.min(), vmax=x_true.max())
        ...
        cax = divider.append_axes("right", size="5%", pad=0.05)
        fig.colorbar(im, cax=cax, orientation="vertical")
    im = ax[1, 1].imshow(x_mean, cmap=cmap, vmin=x_true.min(), vmax=x_true.max())
    ...
    cax = divider.append_axes("right", size="5%", pad=0.05)
    fig.colorbar(im, cax=cax, orientation="vertical")
    im = ax[1, 2].imshow(x_std, cmap=cmap, vmin=0, vmax=x_true.max()-x_true.min())
