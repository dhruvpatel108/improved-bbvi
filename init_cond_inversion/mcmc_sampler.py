...
batched_prior_mean_vec, batched_prior_var_vec, batched_like_var_vec, batched_y_meas, xi = data_processing(
    config.xi_sampling, config.z_dim, 1, config.prior_mean_vec, config.prior_var_vec, config.like_var_vec, config.y_meas)
...
mcmc_post_processing(z_post_samples, f"mcmc_seed{config.seed_no}")
print(f"Time taken = {time.time()-t1} seconds | {((time.time()-t1)/60)} minutes")
