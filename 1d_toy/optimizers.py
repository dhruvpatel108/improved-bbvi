...
# ==================== OPTIMIZATION ====================
# gradient descent
def gradient_descent(xi, xi_orig_1d, mu_init, gamma_init, n_iter, mu_step_size, gamma_step_size, prior, likelihood, if_fd_correction, if_cv):
    mu_list = []
    gamma_list = []
    ...
    for t in range(n_iter):
        ...
        #print(f"iter = {t}, mu_t[0] = {mu_t[0, 0]:.4f}, mu_t[1] = {mu_t[1, 0]:.4f}, gamma_t[0] = {gamma_t[0, 0]:.4f}, gamma_t[1] = {gamma_t[1, 0]:.4f}")
        
    return mu_t, gamma_t, np.array(mu_list).squeeze(), np.array(gamma_list).squeeze()
...
# adam
def adam(xi, xi_orig_1d, mu_init, gamma_init, n_iter, mu_step_size, gamma_step_size, prior, likelihood, if_fd_correction, if_cv,
         b1_mu=0.8, b2_mu=0.8, b1_gamma=0.5, b2_gamma=0.5, eps=1e-8):
    mu_list = []
    gamma_list = []
    mu_t = np.copy(mu_init)
    gamma_t = np.copy(gamma_init)
    ...
    for t in range(n_iter):
        ...
        #print(f"iter = {t}, mu_t[0] = {mu_t[0]:.4f}, mu_t[1] = {mu_t[1]:.4f}, gamma_t[0] = {gamma_t[0]:.4f}, gamma_t[1] = {gamma_t[1]:.4f}")
...
def adam_coordinate_ascent(xi, xi_orig_1d, mu_init, sigma_init, n_iter, mu_step_size, sigma_step_size, prior, likelihood, if_fd_correction, if_cv, 
    ...
    for t in range(n_iter):
        ...
        """
        m_mu = b1_mu*m_mu + (1-b1_mu)*bbvi_grad_mu
        m_sigma = b1_sigma*m_sigma + (1-b1_sigma)*bbvi_grad_sigma
        v_mu = b2_mu*v_mu + (1-b2_mu)*bbvi_grad_mu**2
        v_sigma = b2_sigma*v_sigma + (1-b2_sigma)*(bbvi_grad_sigma**2)
        # bias correction
        m_mu_hat = m_mu/(1-b1_mu**(t+1))
        m_sigma_hat = m_sigma/(1-b1_sigma**(t+1))
        v_mu_hat = v_mu/(1-b2_mu**(t+1))
        v_sigma_hat = v_sigma/(1-b2_sigma**(t+1))

        # gradient ascent
        mu_t = mu_t + mu_step_size*m_mu_hat/(np.sqrt(v_mu_hat) + eps)
        sigma_t = sigma_t + sigma_step_size*m_sigma_hat/(np.sqrt(v_sigma_hat) + eps)        
        print(f"iter = {t}, mu_t[0] = {mu_t[0, 0]:.4f}, mu_t[1] = {mu_t[1, 0]:.4f}, sigma_t[0] = {sigma_t[0, 0]:.4f}, sigma_t[1] = {sigma_t[1, 0]:.4f}")
        """
    return mu_t, sigma_t, np.array(mu_list).squeeze(), np.array(sigma_list).squeeze()
