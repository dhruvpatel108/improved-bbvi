...
def bbvi_gradient(xi, xi_orig_1d, x, mu_t, gamma_t, batched_mu_t, batched_gamma_t, prior, likelihood, if_fd_correction=False, if_cv=False):
    ...
    """
    if if_fd_correction==True and if_cv==True:
        print(f"case 4: if_fd_correction={if_fd_correction} and if_cv={if_cv}")
        grad_mu_fd, grad_sigma_fd = fd_of_log_prior_like_var(xi_orig_1d, config.epsilon, mu_t, sigma_t, prior, likelihood)    # [config.x_dim, 1] each
        first_order_correction_mu = (x - batched_mu_t) @ grad_mu_fd                     # [batch_size, 1]
        first_order_correction_sigma = (x - batched_sigma_t) @ grad_sigma_fd            # [batch_size, 1]
        ...
        # control variate for sigma
        denominator_sigma = np.diag(np.cov(grad_logq_sigma.T))
        f_sigma = grad_logq_sigma * (log_prior_vec + log_like_vec - log_var_vec - first_order_correction_sigma)     # [batch_size, config.x_dim]
        #h_sigma = grad_logq_sigma
        expec_x2_sigma = np.mean(f_sigma*grad_logq_sigma, axis=0)
        expec_squared_sigma = np.mean(f_sigma, axis=0)*np.mean(grad_logq_sigma, axis=0)
        cov_fh_sigma = (expec_x2_sigma - expec_squared_sigma)*(config.batch_size/(config.batch_size-1))
        a_cv_sigma = np.expand_dims(cov_fh_sigma/denominator_sigma, axis=1)

        cv_correction_mu = np.expand_dims(np.mean(grad_logq_mu*a_cv_mu.T, axis=0), axis=1)
        cv_correction_sigma = np.expand_dims(np.mean(grad_logq_sigma*a_cv_sigma.T, axis=0), axis=1)

        # mu gradient
        grad_mu1 = (grad_logq_mu.T @ (log_prior_vec + log_like_vec - log_var_vec - first_order_correction_mu))/config.batch_size
        grad_mu = grad_mu1 - cv_correction_mu + grad_mu_fd
        # sigma gradient
        grad_sigma1 = (grad_logq_sigma.T @ (log_prior_vec + log_like_vec - log_var_vec - first_order_correction_sigma))/config.batch_size
        grad_sigma = grad_sigma1 - cv_correction_sigma 
    """   
...
def fd_gradient(epsilon, mu, gamma, prior, likelihood):
    ...
    xi = np.random.multivariate_normal(mean=np.zeros(config.x_dim), cov=np.eye(config.x_dim), size=batch_size)
    gradL_mu_fd = np.zeros([config.x_dim, 1])
    gradL_gamma_fd = np.zeros([config.x_dim, 1])
    for ii in range(config.x_dim):
        mu_plus = np.copy(mu)
