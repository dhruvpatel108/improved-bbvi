...
class Prior():
    ...
    def log_prior_batch(self, x):
        batched_prior_std_vec = np.sqrt(self.batched_prior_var_vec)
        log_prior = - (np.sum(np.log(batched_prior_std_vec), axis=1)) - (0.5*(np.linalg.norm((x - self.batched_prior_mean_vec)/batched_prior_std_vec, axis=1)**2)) #- (0.5*self.batched_prior_mean_vec.shape[1]*np.log(2*np.pi)) 
        return log_prior
    
    def log_prior_batch_allterms(self, x):
        batched_prior_std_vec = np.sqrt(self.batched_prior_var_vec)
        log_prior = - (0.5*self.batched_prior_mean_vec.shape[1]*np.log(2*np.pi)) - (np.sum(np.log(batched_prior_std_vec), axis=1)) - (0.5*(np.linalg.norm((x - self.batched_prior_mean_vec)/batched_prior_std_vec, axis=1)**2)) 
        return log_prior

    def log_prior_sample(self, x):
        prior_std_vec = np.sqrt(self.prior_var_vec)
        log_prior_sample = - (np.sum(np.log(prior_std_vec))) - (0.5*(np.linalg.norm((x - self.prior_mean_vec)/prior_std_vec)**2)) - (0.5*self.prior_mean_vec.shape[0]*np.log(2*np.pi))
...
class Likelihood():
    def __init__(self, y_meas, batched_y_meas, A, like_var_vec, batched_like_var_vec):
        self.y_meas = y_meas
    ...
    def log_likelihood_batch(self, x):
        ...
        res = self.batched_y_meas - (self.A @ x.T).T
    ...
    def log_likelihood_batch_allterms(self, x):
        batched_like_std_vec = np.sqrt(self.batched_like_var)
        res = self.batched_y_meas - (self.A @ x.T).T
    ...
    def log_likelihood_sample(self, x):
        ...
        res = self.y_meas - (self.A @ x)
        log_like_sample = - (np.sum(np.log(like_std_vec))) - (0.5*np.linalg.norm(res/like_std_vec)**2) - (0.5*self.y_meas.shape[0]*np.log(2*np.pi)) 
